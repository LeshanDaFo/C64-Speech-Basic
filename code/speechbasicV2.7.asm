...
+       LDA #<CBM_CMDTAB                
        ...
        JSR CBM_CHRGET                  ; delete the next char, to avoid a syntax error ???
                                        ; but this only works if there was only one char behind the command
;.loop1 JSR CBM_CHRGET
;       BNE .loop1                      ; this would work better
...
; in the next sequence, a LDA $D3 should also work
...
chkparam                             
        ...
        BCS +                           ; branch if 8 or bigger
        LDX #ILLEGAL_DEVICE_NUMBER      ; else do illegal device error
        JMP CBM_ERROR                   ; Error handling
---------------------------------
...
GETADDR
        ...
        CPX #$F9
        BCC +
        JMP illqu_error                 ; do an error if it is equal or higher
---------------------------------
+       RTS
---------------------------------
...
; the next command is double, it is also called at L0E36
        JSR CBM_CLRCHN                  ; Restore I/O Vector
...
exec_pause
        ...
        STA $0315                       ; address
        CLI                             ;
; In the next part there is a small problem when the low byte of the timer is close to zero, or allready zero.
; While checking the LOW byte, the HIGH byte may already have been counted down in the IRQ.
; If this is the case, 255 cycles of the LOW byte are skipped and not longer counted.
; 
; the solution is to check the high byte first.
-       LDA timer                       ; compare
        BNE -                           ;
        LDA timer+1                     ; 'timer'
        BNE -                           ; loop
...
out_4   JSR out_1
        JSR out_1
        JSR out_1
; the last jump is not necessary
        JMP out_1
---------------------------------
; - $15DB out_1
